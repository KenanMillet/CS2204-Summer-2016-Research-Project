...
//    void updateDisplays(const QStringList& dispListData);
    ...
    // Updates the display that was played on
    void updateDisplayRed(const QString& displaySelected);

    // Updates the displays so that they are all black
    void updateDisplaysBlack();

    // Updates the displays colors to show if a display was played on in the previous turn
    void updateDisplayColor(const QString& p1played, const QString& p2played,
...
private:
    Ui::MainWindow *ui;
    WorkerThread *t;
    QString display0, display1, display2, display3;
