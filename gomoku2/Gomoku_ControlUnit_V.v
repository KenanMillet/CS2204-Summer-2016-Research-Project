...
module edgeDetect(signal, clk, rising, falling);
	 input signal, clk;
	 ...
	 always@(*) begin
		  rising <= signal & ~signalOld;
		  falling <= signalOld & ~signal;
	 end
endmodule
...
module Gomoku_ControlUnit_V(clk,
   ...
   output p2sturn;
	...
	edgeDetect startED(start, clk, startEdge[0], startEdge[1]);
...
module controlUnit(clk, pause, check, player, p2sturn, p1sturn,
p1played, p2played, valid, p2canplay, reset, start, confirm);
    input clk, pause, p1played, p2played, p2canplay, reset, start, valid;
    ...
    always @(posedge clk or negedge reset or posedge pause) begin
		if(reset == 1'b0) begin
			state <= STATE0;
		end
		if(pause == 1'b0) begin
